/*
  Functions used by the TreeTag (wl:tree). Handle tree display and navigation
...
function startDisplayPopup(nodeId, popupShowId, event)
{
   ...
   {
       element.style.backgroundColor = '#E7EFFC';
   }
...
function scrollToElement()
{
  ...
  if(selectedNodeId != null)
  {
    ...
    if(sElement != null && sElement.offsetWidth > 0 && sElement.offsetHeight > 0)
    {
      sElement.focus();
    }
  }
...
function writeNode(node, icon, highlighted, className, accessibility )
{
   ...
   if(ish)
   {
      //alert(node.nodeId + " is highlighted");
      selectedNodeId = node.nodeId;
      document.write('<span class="tree_sel">');
   }
   ...
   if (node.nodeUrl != null)
     document.write('<a ' + altTextString + ' class="' + class_name  + '" id="link' + node.nId + '" href="' + escapeQuotes(node.nodeUrl) + '"  onKeyPress="checkKeyStroke(event);" ' + mouseOverString +  mouseOutString  + contextMenuString + '/>');
   ...
   for (var n=0; n<nodes.length; n++)
   {
        if(nodes[n].taxonomyByName == nodePath)
        {
            ...
            if(isLocal)
               ...
               //ocServer(nodes[n].nodeId, nodes[n].nodeName);
	...
	for (var n=0; n<parentNode.children.length; n++)
	{
		...
		if (hcn)
		{
			...
			if(isLocal == true)
			{
			   ...
			   callFunction ="oc('";
			   //callFunction="ocServer('";
			}

            var titleText = node.nodeName;
			if (ls)
			{
				document.write('<a href="#" onclick="' + callFunction + node.nId + '\',\'' + escapeQuotes(node.nodeName) + '\', 1);return false;"><img border=0 id="join' + node.nId + '" src="');

				if (ino) {
				   document.write(tree_image_base + 'minus');
				   titleText = titleText + " " + messageCatalog["tree.popup.collapsenode.label"];
				} else {
	               document.write(tree_image_base + 'plus');
	               titleText = titleText + " " + messageCatalog["tree.popup.expandnode.label"];
				}

				document.write('bottom.gif" align="absbottom" title="' + titleText + '" alt="' + titleText + ' " /></a>');
			} else
			{
				document.write('<a href="#" onclick="' + callFunction + node.nId + '\',\'' + escapeQuotes(node.nodeName) + '\', 0);return false;"><img border=0 id="join' + node.nId + '" src="');
				if (ino) {
					document.write(tree_image_base + 'minus');
					titleText = titleText + " " + messageCatalog["tree.popup.collapsenode.label"];
				} else {
		...
		var altInfo = node.nodeName + ", " + messageCatalog["tree.popup.level.label"] + " " + recursedNodes.length + ", ";
		if(hcn)
		{
		   if(ino)
		     altInfo += messageCatalog["tree.popup.expanded.label"] + ", ";
		   else
		     altInfo += messageCatalog["tree.popup.collapsed.label"] + ", ";
		}
		altInfo +=  + (n+1) + " " + messageCatalog["tree.popup.of.label"] + " " + parentNode.children.length;

		if(ish)
		  altInfo += ", " + messageCatalog["tree.popup.selected.label"];
		...
		if (hcn)
		{
			document.write('<div id="div' + node.nId + '"');
 			//if (!ino) document.write(' style="display: none;border: 1px solid black;"');
...
// Opens or closes a node
// nodeId = the escaped Node Id
function ocServer(nodeId, nodeName)
{
	...
	if (theJoin.src.indexOf('plus.gif') != -1 || theJoin.src.indexOf('plusbottom.gif') != -1) // node is to be opened
	{
		...
		document.HierarchyManagementForm.nodeId.value = nodeId;
        //document.HierarchyManagementForm.submit();
        open(nodeId, nodeName, document.HierarchyManagementForm.action.value);
...
function ocLocal(nodeId, nodeName, bottom)
{
	...
	var theLink = document.getElementById('link' + nodeId);

	var titles = theLink.title.split(", ");
	...
	// test
	//alert(openNodes[nodeId] + theJoin.src);

	if (!isLocal)
       storeOpenNodesInCookie();
    else
       storeOpenNodesInServer();
	...
	if (theJoin.src.indexOf('plus.gif') != -1 || theJoin.src.indexOf('plusbottom.gif') != -1) // node is to be opened
	{
		...
		altTitleText = nodeName + " " + messageCatalog["tree.popup.collapsenode.label"];
		theJoin.title = altTitleText;
		theJoin.alt   = altTitleText;
		titles[2] = messageCatalog["tree.popup.expanded.label"];
	...
	else
	{
		...
		altTitleText = nodeName + " " + messageCatalog["tree.popup.expandnode.label"];
		theJoin.title = altTitleText;
		theJoin.alt   = altTitleText;
		titles[2] = messageCatalog["tree.popup.collapsed.label"];
		theIcon.src = getNode(nodeId).iconSrcClosed;
		theDiv.style.display = 'none';
	}

	 theLink.title = titles.join(", ");
}
...
// store the openNodes in the cookie
function storeOpenNodesInServer()
{
    var cookieName = "openNodes";
    var cookieValue = "";
    ...
    //document.cookie = cookieName + "=" + escape(cookieValue);
    saveOnServer(cookieName+"="+escape(cookieValue));
}
...
// retrieve openNodes from cookie
function retrieveOpenNodesFromServer()
{

   //var arr = document.cookie.split(";");
   var arr = null;

   //var val = getOnServer();    // async request
   var val = tNodes;
...
{
   td.style.backgroundColor='#E7EFFC';
...
// To do this you have to set an ID on your form called "HierarchyManagementForm"
function addExtraParameters(param, value)
{
   var treeForm = document.getElementById('HierarchyManagementForm');
